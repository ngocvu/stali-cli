...
/** Menu wizard riêng cho Qwen Code (security.auth OpenAI). */
export const QwenDetailMenu: React.FC<{
  ...
  const shortcuts = getProtocolModelShortcuts("qwen");

  const items: { label: string; value: QwenMenuAction }[] = [
    { label: "⚡ Quick setup (stali/qwen3-codex)", value: "quick-setup" },
    ...shortcuts.map((m) => ({
      label: `🎯 Dùng model: ${m}`,
      value: `use-model:${m}` as QwenMenuAction,
    })),
  ...
  return (
    <Card title="🟣 CẤU HÌNH QWEN CODE" borderColor="magenta">
      <Box flexDirection="column" gap={1}>
        ...
        <Text color="gray">
          File: <Text color="white">~/.qwen/settings.json</Text>
        </Text>
        <Text color="gray">
          Patch: security.auth (openai), model.name
        </Text>
