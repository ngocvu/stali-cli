...
async function main() {
  ...
  assert("--version = 2.2.0", ver.stdout?.trim() === "2.2.0", ver.stdout?.trim());
  ...
  assert("bin/stali.js chạy được", binCli.stdout?.trim() === "2.2.0");
  ...
  assert("info --json", info.ok && /"version"\s*:\s*"2.2.0"/.test(info.detail || ""));

  const initSkip = run("init --skip-configure", ["init", "--skip-configure", "-k", rawKey], envHome);
  assert("init --skip-configure", initSkip.ok, initSkip.detail?.slice(0, 200));

  const pluginsInit = run("plugins --init", ["plugins", "--init"], envHome);
  assert("plugins --init", pluginsInit.ok && /plugins\.json/.test(pluginsInit.detail || ""));

  const langEn = run("check --lang en", ["--lang", "en", "check"], envHome);
  assert("check --lang en", langEn.ok || /Action required|OK/.test(langEn.detail || ""));
  ...
  const doctorFixDry = run("doctor --fix dry-run", ["doctor", "--fix", "--dry-run", "-k", rawKey], envHome);
  assert("doctor --fix dry-run", doctorFixDry.ok, doctorFixDry.detail?.slice(0, 200));
